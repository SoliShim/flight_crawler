...
# (이전 버전의 wait_for_flight_results 함수로 복원)
...
async def scrape_flights_evaluate_fixed(page, max_items_to_scrape=30): # max_items_to_scrape 매개변수 추가
    """
    ...
    scroll_and_scrape_script = f"""
    async () => {{
        const results = [];
        const itemSelector = 'div.combination_ConcurrentItemContainer__uUEbl';
        const maxItems = {max_items_to_scrape}; // Python 변수 주입
